    ...
    Mutation: {
        ...
        deleteTrainee(parent, args, context) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                if (!deleteTrainee)
                    throw new Error(" Trainee doesn't exist");
                const updated = yield traineeAttribute_1.traineEAttributes
                    .findOneAndDelete({ trainee_id: args.id }, (err, res) => {
                    if (err) {
                        console.log(err);
                    }
                })
                    .clone();
                const deletedTrainee = yield traineeApplicant_1.default.findOneAndDelete(args.id).populate("cycle_id");
                return deletedTrainee;
            });
        },
