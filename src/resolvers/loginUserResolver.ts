...
export const loggedUserResolvers: any = {
  Query: {
    async user_Logged(_: any, args: any, ctx: any) {
      ...
      const userWithRole = await LoggedUserModel.findById(ctx.currentUser._id)
        .populate("role")
        .populate("cohort");
    ...
    async getInterviewCoordinators(
      _: any,
      args: any,
      ctx: any,
      amount: number
  ...
  Mutation: {
    ...
    async updateUser_Logged(
      ...
      const wasEdited = (
        await LoggedUserModel.updateOne({ _id: ID }, updateData)
      ).modifiedCount;
      return wasEdited;
    },
    ...
    async updateUserSelf(
      ...
      if (ctx.currentUser._id.toString() !== ID) {
        throw new AuthenticationError(
          "You are only authorized to update your own account."
        );
      }
    ...
    async verifyUser(_: any, { ID }: any) {
      try {
        ...
        await user?.save();
        const message = await sendEmailTemplate(
          email,
          " Welcome to Devpulse – Your Account is Ready!",
          `Hello ${email.split("@")[0]},`,
          `
